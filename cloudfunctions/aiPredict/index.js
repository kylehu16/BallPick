...
async function callAIModel(match, ai) {
  ...
  const systemPrompt = `你是专业的足球比赛预测专家。请严格按照以下两个维度进行分析，最终给出预测。

【核心原则：基本面为主，赔率与凯利指数为辅】
- 基本面分析（维度一）是预测的绝对核心，占决策权重的 70%
- 赔率与凯利指数分析（维度二）占 30%，用于辅助验证和微调
...
5. 离散度与风险评估
   - 同一结果多家机构凯利指数标准差越小，机构共识度越高
   - 若离散度 < 0.03 且方向一致，该方向可信度显著提升
...
【思考链要求：你必须将整个分析推理过程详细记录在 thinking 字段中，展示你的逐步思考过程】

请按照以下步骤进行思考，并将完整思考过程填入 thinking 字段：
步骤1 - 基本面分析：逐一分析阵容、风格、伤病、环境、主客场、赛事规则的各项发现
步骤2 - 赔率与凯利分析：逐一分析赔率概率、凯利指数值域、离散度等各项发现
步骤3 - 两维度对比：基本面结论与赔率凯利方向是否一致？若有冲突如何权衡？
步骤4 - 方案生成与评估：列出生成的10个方案，逐一评估置信度并说明理由
步骤5 - 最终选择：说明为何选择当前方案作为置信度最高的结果
...
必须只返回一个 JSON 对象，格式如下：
{
  ...
  "awayScore": 1,
  "confidence": 75,
  "thinking": "【步骤1-基本面分析】阵容：... | 风格：... | 伤病：... | 环境：... | 主客场：... | 赛事规则：... 【步骤2-赔率与凯利分析】赔率：... | 凯利：... 【步骤3-两维度对比】... 【步骤4-方案评估】方案1（主胜2-1，置信度75）：... 方案2（...）... 【步骤5-最终选择】综合基本面（权重70%）与赔率凯利（权重30%），选择方案1...",
  "oddsAnalysis": "赔率与凯利指数维度总结：主胜赔率1.85隐含概率51%，平赔3.50客胜4.20，市场倾向主队；竞彩主胜凯利0.94、平局0.88、客胜0.97...",
  "altitudeImpact": "海拔影响分析（无高海拔差异则填'无'）",
  "weatherImpact": "天气影响分析",
  "lineupNotes": "基本面维度阵容总结：双方阵型、核心球员状态、伤病禁赛及风格分析",
  "reasoning": "以基本面主结论为核心，赔率与凯利指数辅助验证：解释为何该方案置信度最高，若两维度有冲突在此说明"
}`;

  const userPrompt = `请预测以下比赛，请一步步思考，将完整推理过程填入 thinking 字段：
...
请严格按系统指令中的5个步骤（基本面→赔率凯利→两维度对比→方案评估→最终选择），搜索以上比赛的全面信息，生成10个不同的预测方案，只返回置信度最高的 JSON 结果（必须包含 thinking 字段记录完整推理过程）。`;
